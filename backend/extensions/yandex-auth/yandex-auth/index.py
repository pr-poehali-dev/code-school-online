        ...
        try:
            ...
            if row:
                ...
                if email and row:
                    ...
                    # 3. Create new user
                    # email column is NOT NULL in this project — use a placeholder if Yandex gave none
                    new_email = email or f'yandex_{yandex_id}@yandex.local'
                    cur.execute(
                        f"""INSERT INTO {S}users
                            (yandex_id, email, name, avatar_url, email_verified, created_at, updated_at, last_login_at)
                            VALUES (%s, %s, %s, %s, TRUE, %s, %s, %s)
                            RETURNING id""",
                        (yandex_id, new_email, name or 'Ученик', picture, now, now, now)
                    )
