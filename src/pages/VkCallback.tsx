...
const VkCallback = () => {
  ...
  useEffect(() => {
    ...
    vkAuthApi
      ...
      .catch((e: Error) => setError(e.message || 'Не удалось войти через VK'));
  }, [login, navigate]);
      ...
      <div className="glass rounded-2xl p-8 text-center max-w-sm w-full">
        {error ? (
          ...
          <>
            ...
            <p className="text-sm text-muted-foreground">Входим через ВКонтакте...</p>
          </>
        )}
      </div>
    </div>
